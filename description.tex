...
transit duration, or physically, the actual to circular velocity.
Note that in the ratio $T/T_c$, the minimum impact parameter cancels
out (although it is explicitly included in the transit model and will
affect other parameters through covariance).  This leaves $e_{min}$ a
function of 4 parameters: $P$, $a$, $v$, and $R_*$.  We will use
the \kepler lightcurves to constrain $P$ and $v/R_*$, meaning that we
must determine though other means the semi--major axis and stellar
radius.  The semi--major axis, and also the reference circular
...
typically the most poorly constrained of all model parameters, because
its primary constraint comes from the ingress/egress durations, which
are poorly resolved for small planets at a 30--minute cadence.
However, with now several hundred transits per system to (each poorly)
resolve the ingress/egress, our KOI 701.01 analysis below suggests
that the dependence of other model parameters on $b$ does not
compromise our $e_{min}$ analysis, although we have not fully tested
this at large impact parameter ($b \sim 1$).
...
velocity planets \citep{Plavchan12,Kane13}. These studies made good
progress, but were limited by the number of known candidates, as well
as the relatively poor characterization of the transits themselves.
...
or, physically,
\begin{eqnarray}
e_{min} & = & \left| \frac{1 - (v_c/v_\perp)^2}{1 + (v_c/v_\perp)^2} \right| .
\label{eq-emin3}
\end{eqnarray}
