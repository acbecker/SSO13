...
\resizebox{2.7in}{!}{\includegraphics{figures/compq.ps}} 
\end{minipage}
\begin{minipage}{2.7in}
...
\centering
\begin{minipage}{2.7in}
\resizebox{2.7in}{!}{\includegraphics{figures/radper.ps}} 
\end{minipage}
\begin{minipage}{2.7in}
...
\centering
\begin{minipage}{2.7in}
\resizebox{2.7in}{!}{\includegraphics{figures/compare.emin.ps}} 
\end{minipage}
\begin{minipage}{2.7in}
...
and uncertainties.  We were able to reduce the first 32 transits in
...
the time of each transit, for all $N = 32$ transits.  This means that
for transit $n \leq N$, we have common model parameters
$\beta_{0}^2, \tau, \zeta$ and per--transit parameters $t_{0;i=1..n}$,
for a total of $n+3$ model parameters.  This yields an ensemble of $N$
system models per lightcurve, each incorporating one more transit than
the previous one.
...
0.0182_{0.0003}^{0.0006}$ vs \cite{2013arXiv1304.7387B} $\zeta =
0.0188 \pm 0.0003$.
...
signal--to--noise, we generated simulated lightcurves at the \kepler
cadence.  We used a subset of 6 of the synthetic systems described in
the first section of this proposal.  These were chosen to have transit
depths of [5e-5/1e-4/5e-4/1e-3/5e-3/1e-2] precent.  We use the system
inclination, semi--major axis, planet--to--star radius ratio, and
period (along with two limb darkening parameters) to generate fake
system lightcurves using the method of
...
in the \kepler data.  We then model these data using the
parameterization described above.  Figure~\ref{fig-dtau} outlines how
our signal--to--noise on $\tau$ scales with the system brightness and
...
entirely consistent with our more complete results from KOI 701.01,
where the host star's brightness is 13.6, transit depth is 0.04\%, and
measured signal--to--noise in $\tau$ is approximately 8 after modeling
10 transits: $0.0479_{0.0049}^{0.0073}$.  Note this precision
increases to a signal--to--noise of nearly 20 after 30 transits.
...
\begin{figure*}[t] 
  ...
  \begin{minipage}[c]{0.5\textwidth}
    ...
    star brightness and transit depth.  The host star brightness sets
    the signal--to--noise of the photometry.  These values were
    obtainined after fitting 10 tranits jointly, using the geometric
    parameterization described within this proposal.  These
...
Figure~\ref{fig:emin} indicates that this project {\it requires} a
